...
for (let i = 0; i < smoothScrollTrigger.length; i++) {
    smoothScrollTrigger[i].addEventListener('click', (e) => {
        ...
        href = document.getElementById(href.replace('#', '')); // リンク先の要素（コンテンツ）を取得
        let targetElement = document.getElementById(href.replace(new RegExp(nowurl,'g'), '')); // リンク先の要素（コンテンツ）を取得
